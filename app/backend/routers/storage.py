...
def _local_storage_root() -> Path:
    configured = (settings.upload_storage_root or "").strip()
    if configured:
        return Path(configured).expanduser()
    return Path(__file__).resolve().parents[1] / "local_storage"


def _safe_storage_part(value: str, label: str) -> str:
    safe = Path(str(value or "").strip()).name
    if not safe or safe in {".", ".."}:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid {label}")
    return safe


def _local_file_path(bucket_name: str, object_key: str) -> Path:
    bucket = _safe_storage_part(bucket_name, "bucket_name")
    key = _safe_storage_part(object_key, "object_key")
...
async def local_upload_file(bucket_name: str, object_key: str, file: UploadFile = File(...)):
    ...
    """
    try:
        target = _local_file_path(bucket_name, object_key)
...
async def local_download_file(bucket_name: str, object_key: str):
    ...
    """
    try:
        file_path = _local_file_path(bucket_name, object_key)
