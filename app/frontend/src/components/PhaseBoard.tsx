    ...
    <div className="flex min-h-0 min-w-0 w-full flex-1 flex-row flex-nowrap gap-4 overflow-x-auto overflow-y-hidden snap-x snap-mandatory [-webkit-overflow-scrolling:touch]">
      {phases.map((phase) => {
        ...
        return (
          <div
            key={phase.key}
            className="flex min-h-0 min-w-0 flex-shrink-0 flex-col snap-start w-[min(20rem,calc(100%-2rem))] sm:w-[21rem] lg:w-[clamp(18rem,22vw,26rem)] overflow-hidden rounded-lg border border-border/50 shadow-sm transition-[box-shadow,border-color] duration-200 ease-out hover:border-border hover:shadow-md"
